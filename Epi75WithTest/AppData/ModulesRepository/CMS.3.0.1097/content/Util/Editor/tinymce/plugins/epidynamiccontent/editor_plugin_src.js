(function (tinymce, $) {
    ...
    tinymce.create('tinymce.plugins.epidynamiccontent', {
                ...
                ed.onNodeChange.addToTop(function (ed, cm, n, co, options) {
                    ...
                    if (c) {
                        ...
                        if (tinymce.isIE && co && (ed.dom.hasClass(n, "epi_dc_editBtn") || ed.dom.hasClass(n, "epi_dc_previewBtn"))) {
                            t._handleButtonsClick({ target: n, button: 0 });
                        }
        ...
        */
        ...
        /**
        * Focus on a DC regarding to the event target
        *
        * @param {Event} e The event object.
        */
        _focusSelection: function (e) {
            // Try to collapse selection before getting focus on the new one.
            // If not, we will get an error of invalid argument in this.ed.focus().
            if (tinymce.isIE ||  this.ed.selection.getRng().collapsed) {
            ...
            this.ed.focus();
            var selection = this._getParentContentBlock(e.target);
            this.ed.selection.select(selection);
        },
        ...
        */
        _createPopupIframe: function (popupPosition, frameName, width, buttons) {
            ...
            if (previewBorder.length === 0) {
                ...
                // Need to bind mouse down instead of click because click event may be captured by IframeWithOverlay first
                $(window.document.body).mousedown(function () { t._hidePreview(); });
            }
