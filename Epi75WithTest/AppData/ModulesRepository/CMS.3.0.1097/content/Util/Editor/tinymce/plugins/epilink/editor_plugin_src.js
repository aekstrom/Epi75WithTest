        ...
        */
        init: function (ed, url) {
            ...
            ed.addCommand('mceEPiLink', function () {
                ...
                // Workaround when no selection and is IE,
                // cause by:    win.getSelection return wrong dom node:
                //              Paragraph alternative for Anchor element
                if (tinymce.isIE &&                                     // Just occur on IE
                    !selectedLink &&                                    // Can not select link
                    node.tagName === 'P' &&                             // Was wrapped by paragraph
                    node.childNodes.length === 1 &&                     // Has only one direct child
                    node.childNodes[0].tagName === 'A') {               // Child is type of anchor tag
                    selectedLink = node.childNodes[0];
                }
