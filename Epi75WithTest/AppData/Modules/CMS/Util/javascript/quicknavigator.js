(function (scope) {
    ...
    var template = '<li class="epi-quickNavigator-editLink"><a href="{0}"><span>{episervertext}</span></a></li> \
    ...
    scope.QuickNavigator = function (settings) {
        ...
        if (window.self !== window.top) { return; } // we're being framed, Abort!
        ...
        this._listeners.push(on(window, "load", function () { self.initialize(); }));
        this._listeners.push(on(window, "unload", function () { self.destroy(true); }));
    };
        ...
        _buildMenuContent: function (menuNode) {
            ...
            for (key in menuItems) {
                if (menuItems.hasOwnProperty(key)) {
                    alternatives += '<li><a href="' + menuItems[key].url + '">' + menuItems[key].caption + '</a></li>';
                }
            }
