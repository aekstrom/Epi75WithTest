            ...
            <div class="epi-formArea">
                ...
                <div class="epi-size10">
                    ...
                    <asp:Panel runat="server">
                        ...
                        <asp:RegularExpressionValidator runat="server"  ValidationExpression="^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*" Text="*" ErrorMessage="<%$ Resources: EPiServer, admin.edituser.invalidemail %>" ID="EmailValidator" ControlToValidate="Email" Display="Dynamic"  EnableClientScript="false"/>
...
<asp:PlaceHolder ID="ToolbarPanel" runat="server">
    <div class="epi-buttonContainer">
        <EPiServerUI:ToolButton id="DeleteButton" 
        OnClientClick='<%#"OpenDeleteDialog(\"" + MembershipUser.Value + "\", 0, \"" + Server.UrlEncode(MembershipProviderName) + "\")"%>' GeneratesPostback="false" skinid="Delete" visible="<%# IsAdminMode %>" text="<%$ Resources: EPiServer, button.delete %>" tooltip="<%$ Resources: EPiServer, button.delete %>" runat="server" /><EPiServerUI:ToolButton id="ApplyButton" onclick="SaveButton_Click" skinid="Save" text="<%$ Resources: EPiServer, button.save %>" tooltip="<%$ Resources: EPiServer, button.save %>" runat="server" />
    </div>
</asp:PlaceHolder>
