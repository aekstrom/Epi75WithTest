...
function Initialize(isEnabled)
{
    ...
    if (EPi.GetDialog())
    {
        ...
        {
            LoadImages(dialogArguments.src);
        }
...
function LoadImages(imagePath) {
    ...
    if (_imageEditorEnabled) {
        ...
        var sImageSource = "RenderImage.ashx?zoom=" + _image.zoom + "&quality=" + sQuality.value + "&img=" + uniqueImagePath;
        sImageSource += "&commands=" + commandQueue.Serialize();

        selectionImage.src = sImageSource;
        realImage.src = sImageSource;
    }
    else {
